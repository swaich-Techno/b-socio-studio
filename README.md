...
- Google Maps lead discovery pipeline with lead scoring, outreach status, and convert-to-client action
...
- Reel script studio plus video-provider job adapter for evergreen/timeless reel creation
...
This codebase still runs on MongoDB/Mongoose until the route conversion is completed. A migration scaffold is included at `prisma/schema.prisma`, and the exact checklist is in `docs/NEON_MIGRATION.md`. Use Neon pooled `DATABASE_URL` for Vercel runtime and direct `DIRECT_URL` for migrations.
