...
export function generateContentPlan(input, client = {}) {
  ...
  const requestedLanguage = input.language || brandBrain.preferredLanguage || "English";
  const selectedLanguage = ["Hindi", "Punjabi"].includes(requestedLanguage) ? "Hinglish" : requestedLanguage;
...
export function generateReelPlan(input, client = {}) {
  ...
  const requestedLanguage = input.language || brandBrain.preferredLanguage || "Hinglish";
  const language = ["Hindi", "Punjabi"].includes(requestedLanguage) ? "Hinglish" : requestedLanguage;
