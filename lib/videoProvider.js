function secondsFromDuration(value) {
  const match = String(value || "10").match(/\d+/);
  const seconds = match ? Number(match[0]) : 10;
...
export async function createVideoJob({ reel, prompt }) {
  const apiKey = process.env.RUNWAY_API_KEY || process.env.RUNWAYML_API_SECRET || process.env.VIDEO_API_KEY;
  const endpoint = process.env.REEL_VIDEO_API_ENDPOINT || process.env.RUNWAY_VIDEO_API_ENDPOINT || "https://api.dev.runwayml.com/v1/image_to_video";
  ...
  if (!apiKey) {
    return {
      configured: false,
      provider,
      message: "Add RUNWAY_API_KEY in Vercel to enable video job creation."
    };
  }

  const payload = {
    model: process.env.REEL_VIDEO_MODEL || "gen4.5",
    promptText: prompt,
    ratio: process.env.REEL_VIDEO_RATIO || "720:1280",
