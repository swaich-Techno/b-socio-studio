...
export async function GET(request) {
  ...
  const invoices = await Invoice.find({ clientId: { $in: clientIds } }).populate("clientId", "businessName email whatsappNumber phone").populate("packageId", "name").sort({ createdAt: -1 }).lean();
