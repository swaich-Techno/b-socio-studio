...
export async function GET(request, context) {
  ...
  const invoice = await Invoice.findOne({ _id: id, clientId: { $in: clientIds } }).populate("clientId", "businessName industry location contactPerson email phone whatsappNumber").populate("packageId", "name").lean();
...
export async function PUT(request, context) {
  try {
    ...
    const clientIds = await accessibleClientIds(auth.user);
    const invoice = await Invoice.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, normalizeInvoice(body, auth.user), { new: true, runValidators: true });
    if (!invoice) return NextResponse.json({ error: "Invoice not found." }, { status: 404 });
...
export async function DELETE(request, context) {
  ...
  const { id } = await context.params;
  const clientIds = await accessibleClientIds(auth.user);
  const invoice = await Invoice.findOneAndDelete({ _id: id, clientId: { $in: clientIds } });
  if (!invoice) return NextResponse.json({ error: "Invoice not found." }, { status: 404 });
