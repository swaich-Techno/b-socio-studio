...
export async function PUT(request, context) {
  try {
    ...
    const reel = await Reel.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, body, { new: true, runValidators: true });
...
export async function DELETE(request, context) {
  ...
  const { id } = await context.params;
  const clientIds = await accessibleClientIds(auth.user);
  const reel = await Reel.findOneAndDelete({ _id: id, clientId: { $in: clientIds } });
  if (!reel) return NextResponse.json({ error: "Reel not found." }, { status: 404 });
