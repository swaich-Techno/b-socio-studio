...
export async function PUT(request, context) {
  try {
    ...
    if (!existing || !(await canAccessClient(auth.user, existing.clientId))) return NextResponse.json({ error: "Task not found." }, { status: 404 });
    ...
    const update = canUpdateAssignedTask && !isOwnerAdmin(auth.user) && !hasPermission(auth.user, "assign_tasks")
      ...
      : body;
    const task = await Task.findOneAndUpdate({ _id: id, clientId: existing.clientId }, update, { new: true, runValidators: true });
...
export async function DELETE(request, context) {
  ...
  const { id } = await context.params;
  const existing = await Task.findById(id).select("clientId").lean();
  if (!existing || !(await canAccessClient(auth.user, existing.clientId))) return NextResponse.json({ error: "Task not found." }, { status: 404 });
  const task = await Task.findOneAndDelete({ _id: id, clientId: existing.clientId });
  if (!task) return NextResponse.json({ error: "Task not found." }, { status: 404 });
