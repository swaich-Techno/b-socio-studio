...
export async function PUT(request, context) {
  try {
    ...
    const member = await User.findOneAndUpdate({ _id: id, agencyName: auth.user.agencyName }, update, { new: true, runValidators: true }).select("-password");
    if (!member) return NextResponse.json({ error: "Team member not found." }, { status: 404 });
...
export async function DELETE(request, context) {
  ...
  const member = await User.findOneAndDelete({ _id: id, agencyName: auth.user.agencyName });
  if (!member) return NextResponse.json({ error: "Team member not found." }, { status: 404 });
