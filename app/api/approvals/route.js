...
export async function GET(request) {
  ...
  const [calendarApprovals, contentApprovals, reelApprovals, clientApprovals, pendingUsers] = await Promise.all([
    ...
    isOwnerAdmin(auth.user) ? User.find({ agencyName: auth.user.agencyName, status: "pending" }).select("-password").sort({ createdAt: -1 }).lean() : []
  ]);
...
export async function PUT(request) {
  try {
    ...
    if (type === "user") {
      ...
      const target = await User.findOne({ _id: id, agencyName: auth.user.agencyName }).select("-password");
    ...
    if (type === "calendar") {
      const item = await Calendar.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, { approvalStatus: approve ? "Approved" : "Client Revision", status: approve ? "Approved" : "Client Revision" }, { new: true });
      return NextResponse.json({ item });
    }

    if (type === "content") {
      const item = await Content.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, { approvalStatus: approve ? "Approved" : "Revision Needed" }, { new: true });
      return NextResponse.json({ item });
    }

    if (type === "reel") {
      const item = await Reel.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, { status: approve ? "Approved" : "Idea" }, { new: true });
      return NextResponse.json({ item });
    }

    if (type === "clientApproval") {
      const item = await ClientApproval.findOneAndUpdate({ _id: id, clientId: { $in: clientIds } }, { status: approve ? "Approved" : "Revision Needed", clientComments: body.clientComments || "" }, { new: true });
      return NextResponse.json({ item });
    }
